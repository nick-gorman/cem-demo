...
# Streamlit app
st.title('Energy System Optimisation Modelling')
...
st.markdown("The house hold energy use could be modelled individually (representative sample).")
...
st.markdown("However, demand is typically represented at a regionally aggregated level. Either by aggregating modelled"
...
# EV Charging Section
st.subheader('1.3 EV Charging Profiles')
st.markdown("New technologies, such as EVs, are often modelled as distinct new loads that are added to the aggregate "
            "demand.")
...
st.subheader('2.1 Flexible EV Charging')
st.markdown("If loads such as EVs are modelled dynamically interacting with the broader system, constraints are used "
            "to enforce behavioural assumption, but the final load profile is determined at model runtime.")
